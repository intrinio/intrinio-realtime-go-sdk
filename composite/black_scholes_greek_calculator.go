...
func (b *BlackScholesGreekCalculator) calcImpliedVolatility(isPut bool, underlyingPrice, strike float64, yearsToExpiration float64, riskFreeInterestRate, dividendYield, marketPrice float64) float64 {
	...
	for iter := 0; iter < maxIter; iter++ {
		var price float64
		if isPut {
			price = b.calcPricePut(underlyingPrice, strike, yearsToExpiration, riskFreeInterestRate, sigma, dividendYield)
		} else {
			price = b.calcPriceCall(underlyingPrice, strike, yearsToExpiration, riskFreeInterestRate, sigma, dividendYield)
		}
...
func (b *BlackScholesGreekCalculator) calcGamma(underlyingPrice, strike float64, yearsToExpiration float64, riskFreeInterestRate, dividendYield, marketPrice, sigma float64) float64 {
	d1 := b.d1(underlyingPrice, strike, yearsToExpiration, riskFreeInterestRate, sigma, dividendYield)
	return math.Exp(-dividendYield*yearsToExpiration) * b.normalPdf(d1) / (underlyingPrice * sigma * math.Sqrt(yearsToExpiration))
}
...
func (b *BlackScholesGreekCalculator) calcVega(underlyingPrice, strike float64, yearsToExpiration float64, riskFreeInterestRate, dividendYield, marketPrice, sigma float64) float64 {
	d1 := b.d1(underlyingPrice, strike, yearsToExpiration, riskFreeInterestRate, sigma, dividendYield)
	return underlyingPrice * math.Exp(-dividendYield*yearsToExpiration) * b.normalPdf(d1) * math.Sqrt(yearsToExpiration) / 100.0
}
...
// cumulativeNormalDistribution calculates the cumulative normal distribution
func (b *BlackScholesGreekCalculator) cumulativeNormalDistribution(z float64) float64 {
	if z > maxZScore {
		return 1.0
	}
	if z < minZScore {
